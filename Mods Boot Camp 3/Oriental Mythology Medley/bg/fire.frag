...
vec3 flame() {
	//fix this
	float xpart = imageCoord.x;
	float ypart = imageCoord.y+yOffset;
	//
	float clip = 210.0;
	float ypartClip = gl_FragCoord.y/clip+yOffset;
	...
	//
	vec2 coordScaled = 0.01*gl_FragCoord.xy;
	vec3 potision = vec3(coordScaled,0.0);
	...
	//
	vec3 displacePos = vec3(1.0,0.5,1.0)*2.4*potision+time*vec3(0.01,-0.7,1.3);
	vec3 displace3 = vec3(noiseStackUV(displacePos,2,0.4,0.1),0.0);
	//
	vec3 noiseCoord = (vec3(2.0,1.0,1.0)*potision+timing+0.4*displace3)/1.0;
	...
	//
	// smoke
	float smokeNoise = 0.5+snoise(0.4*potision+timing*vec3(1.0,1.0,0.2))/2.0;
	vec3 smoke = vec3(0.3*pow(xfuel,3.0)*pow(ypart,2.0)*(smokeNoise+0.4*(1.0-noise)));
	//
	// sparks
	float sparkGridSize = 30.0;
	vec2 sparkCoord = gl_FragCoord.xy - vec2(2.0,190.0*time);
	sparkCoord = sparkCoord - 30.0*noiseStackUV(0.01*vec3(sparkCoord,30.0*time),1,0.4,0.1);
	sparkCoord += 100.0*flow.xy;
	if (mod(sparkCoord.y/sparkGridSize,2.0)<1.0) sparkCoord.x += 0.5*sparkGridSize;
	vec2 sparkGridIndex = vec2(floor(sparkCoord/sparkGridSize));
	float sparkRandom = prng(sparkGridIndex);
	float sparkLife = min(10.0*(1.0-min((sparkGridIndex.y+(190.0*time/sparkGridSize))/(24.0-20.0*sparkRandom),1.0)),1.0);
	...
	//
	return vec3(max(fire,sparks)+smoke);
}
...
void main()
{
	...
	vec4 keyed = vec4( mix( flame(), c.rgb, c.a) ,1);
	gl_FragColor = keyed*color;
