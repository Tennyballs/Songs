...
vec4 value() {
    ...
    vec2 p = gl_FragCoord.xy * dist / imageSize.xx;
    p.x -= time/3.1;
    ...
    vec3 color = vec3(c * cos(shift * gl_FragCoord.y / imageSize.y));
    color += .05;
    color.r *= .8;
...
void main() {
	...
	//
	//vec3 test = vec3(max(fire,sparks)+smoke);

	//vec3 colorcloud = value().xyz;
	//float dist = pow(distance(test, vec3(0.,0.,0.)),2.);
	//vec4 keyed = vec4( mix( colorcloud,test, dist) , 1.0);
