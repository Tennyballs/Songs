...
#define Res  imageSize.xy
#define Res0 imageSize.xy
#define Res1 vec2(256.0,256.0)
...
{
    vec2 uv=pos/Res0;
    vec4 c1 = texture2D(sampler0,texCoord2imgCoord(uv));
...
{
    vec2 uv=pos/Res1;
    return texture2D(sampler1,vec2(fract(uv.x),fract(uv.y)));
}
...
void main()
{
    vec2 pos=(((imageCoord * imageSize)-Res.xy*.5)/Res.y*Res0.y)+Res0.xy*.5;
    vec2 pos2=pos;
    vec2 pos3=pos;
    vec2 pos4=pos;
    vec2 pos0=pos;
    ...
    col = col*vec3(.93,0.93,0.85)
        *mix(texture2D(sampler2,(imageCoord * imageSize).xy/imageSize.xy).xyz,vec3(1.2),.7)
        +.15*getRand(pos0*2.5).x;
    // vignetting
    float r = length(((imageCoord * imageSize)-imageSize.xy*.5)/imageSize.x);
    float vign = 1.-r*r*r*r;
    
	gl_FragColor = vec4(col*vign,1.0)*color;
	
	//vec3 col3 = texture2D( sampler0, textureCoord ).yzx;
	//gl_FragColor = vec4(col3,1.0)*color;
}
