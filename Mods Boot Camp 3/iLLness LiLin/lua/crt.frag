...
void main()
{
	...
	uv = (uv - 0.5) * 2.0;
	uv.x *= 1.0 + amp*pow(uv.y / PI, 2.0);
	uv.y *= 1.0 + amp*pow(uv.x / PI, 2.0);
	uv = uv * 1.01;
	uv = uv * 0.5 + 0.5;
	...
	col *= 1.0 - sin(PI * uv.x * resolution.x / 1.0) * 0.5 * amp;
	col *= 1.0 - cos(PI * uv.y * resolution.y / 2.0) * 0.5 * amp;

	col *= 1.0 - fract(sin(uv.y + time * 0.5)) * 0.5 * amp;
	//col *= 1.0 - length(0.5 - uv) * 0.6 * amp;
