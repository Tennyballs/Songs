...
//This is a shader I based on FMS_Cat's twister shader. I still don't fully understand how all this works but hey, I managed to do something
 ...
 int vID=gl_VertexID;
				...
				//vert.x=clamp(vert.x,0,400);
					...
					//vert.x+=clamp(sin(vert.y*0.08)*(exp((vert.y-230)*0.05)*1),-100,100); //tää on aika harza
					//vert.x+=sin(vert.y*0.1)*(exp((clamp(vert.y-300,-50,50))*0.05)*10); //tääkin on harza
					
					if(gravity<1.9){
					vert.x+=sin(vert.y*1)*(exp((clamp(vert.y-400,-50,50))*0.05)*10); //tääkin on harza
					};
					if(gravity>.1){
					vert.x+=sin(vert.y*1)*(exp((clamp(-vert.y+80,-50,50))*0.05)*10); //tääkin on harza
					};
