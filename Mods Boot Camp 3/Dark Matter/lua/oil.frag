...
void main(){
	...
	vec2 uv = 1.0-2.0*((imageCoord * imageSize).xy / imageSize.xy);
    uv.y /= imageSize.x/imageSize.y;
    float t = iTime*0.6;
    ...
    #ifdef INVERT
    	gl_FragColor = 1.0-1.2*vec4( vec3(0.5*fbm(uv*val*8.0)+0.02*r(uv.x,uv.y)), 0.0 )*color;
    #else
    	gl_FragColor = 1.6*vec4( vec3(0.5*fbm(uv*val*8.0)+0.02*r(uv.x,uv.y)) + vec3(0,0,0.2), 1.0 )*color;
    #endif
